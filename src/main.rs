...
impl State {
    ...
    fn victory(&mut self, ctx: &mut BTerm) {
        ctx.set_active_console(2);
        ctx.print_color_centered(2, YELLOW, BLACK, "You have obtained");
        ctx.print_color_centered(3, YELLOW, BLACK, "the Amulet of Yala!");
        ctx.print_color_centered(
            5,
            WHITE,
            BLACK,
            "You hold it high above your head, feeling the power \
...
impl GameState for State {
    fn tick(&mut self, ctx: &mut BTerm) {
        ...
        ctx.set_active_console(0);
        self.resources
            .insert(Point::from_tuple(ctx.mouse_pos().into()));
...
fn main() -> BError {
    let context = BTermBuilder::new()
        ...
        .with_simple_console_no_bg(DISPLAY_WIDTH * 2, DISPLAY_HEIGHT * 2, "terminal8x8.png") // (7)
        .build()?;
